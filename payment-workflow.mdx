...
- Mindbricks architects por AI agents that devolopes payment based applications in Mindbricks

- Software architects that develope their own services that will interact with Mindbricks services
...
There may be more than one data objects that represent user debit in the application, for example in an online store application, user may buy add some items to the basket and checkout them, but also in the same application user may have another oppurtunity to buy a prime subscription for extra favors. So these two trades may have different nature and the architect may build seperate busnes logic for them. So in the application tehre may be to order type data objects, one mayhave a name like `order`while the other may have a name `userSubscription`. Both data objects will represent some amount of money to charge user. But in practice most applications have one business logic for user payment, if you think that you need more than one payment logics, first try to combine them in one data object is possible.
...
All these data objects may be considered as an order type data object nad can be joined to Stripe in Mindbricks.
...
So make your design smart to create an order data object instance that has its values aligned with its source data object instance.
...
- a property to reprsent the user who is debited,

- a property to represent the amount / or a relation logic to collect the amount

- a property to represent the currency, or a hardcoded settin may overcome it
...
## Understandign StripeOrder pattern
...
To configure the `stripeOrder` pattern in the `objectSettings` consider following points.
...
Following pattern properties are MScript values that will be eveluated in the context. In these MScripts you can reach your order object like , `this.order`or `this.reservation`etc.
...
Following pattern properties should be directly the related proeprty names of your order data object.
...
In addtion to the `orderStatusProperty`of your own logic, Mindbricks automatically adds a system field to your order data object that manages the status of the payment in with system wording. You can always read this property from your data object instance once the instance is created.
...
Please note that, you dont have to create any data objects for the payment specific logic, you just create your order data object and configure its StripeOrder pattern. All other data objects and apis will be created automatically by Mindbricks.
...
There are some API's that are created automatically as business API arround system payment objects and your order object.
The most important one is start payment API, named like `startTicketPayment`.
You also have business API, to manage the system data objects, like listing payments. These API's are all detailed in the documentation of your application.
You will see the usage and purpose of these API's in the flow descriptions below.
